...
os.makedirs('{args.path}/mixtral', exist_ok=True)
...
def main(args):
    ...
    def write_ouput(domain, topic, response, id):
        f = open(f'{args.path}/mixtral/{domain}_{id}.txt','w')
        f.write(response.encode('ascii', 'ignore').decode('ascii'))
        f.close()

    def generate_helpful_data():
        
        help_df = pd.read_csv('{args.path}/helpful_reviews.tsv', sep='\t')
    ...
    def generate_metareview_data():
        mr_df = pd.read_csv('{args.path}/meta_reviews.tsv', sep='\t')
